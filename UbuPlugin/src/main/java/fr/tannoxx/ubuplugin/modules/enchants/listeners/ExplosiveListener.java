 ...
 * Casse les blocs en 3x3
 * Compatible Fortune/Silk Touch + récupération XP
 */
public class ExplosiveListener implements Listener {
    ...
    public void onBlockBreak(@NotNull BlockBreakEvent event) {
        ...
        boolean hasFortune = module.getConfigManager().getBoolean("enchants.explosive.fortune-compatible", true) &&
                tool.containsEnchantment(Enchantment.FORTUNE);
        ...
        int fortuneLevel = hasFortune ? tool.getEnchantmentLevel(Enchantment.FORTUNE) : 0;
        int unbreakingLevel = tool.getEnchantmentLevel(Enchantment.UNBREAKING);
        boolean isUnbreakable = tool.getItemMeta() != null && tool.getItemMeta().isUnbreakable();

        // Vérifier si le joueur a Experience
        ...
        // Vérifier si le joueur a Magnetic
        ...
        try {
            ...
            for (Block targetBlock : blocksToBreak) {
                ...
                if (hasSilkTouch) {
                    drops = Collections.singletonList(new ItemStack(targetBlock.getType()));
                } else if (hasFortune) {
                    drops = targetBlock.getDrops(tool);
                } else {
                    drops = targetBlock.getDrops(tool);
                }
                ...
                // Gérer l'XP (seulement si pas Silk Touch et collectXP activé)
                if (!hasSilkTouch && collectXP) {
                    int xp = getBlockExperience(targetBlock.getType());
                    if (xp > 0) {
                        // Appliquer bonus Experience
                ...
                // Gérer les drops
                if (hasMagnetic) {
                    // Ajouter directement dans l'inventaire
    ...
    /**
     * Détermine la face du bloc cassée
     */
    ...
    /**
     * Obtient les blocs en 3x3 selon la face
     */
