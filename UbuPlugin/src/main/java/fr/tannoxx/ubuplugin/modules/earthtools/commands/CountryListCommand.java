...
public record CountryListCommand(UbuPlugin plugin) implements CommandExecutor {

    private static final Pattern HEX_PATTERN = Pattern.compile("&(#[A-Fa-f0-9]{6})");

    @Override
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, String[] args) {
        File file = new File(plugin.getDataFolder(), "countrylist.txt");
        if (!file.exists()) {
            sender.sendMessage("⓪ §cAucune liste de pays trouvée / §rⓧ §cNo country list found");
            return true;
        }
        ...
        try {
            ...
            sender.sendMessage("⓪ §cErreur lors de la lecture du fichier / §rⓧ §cError reading file");
        }
        return true;
    }
