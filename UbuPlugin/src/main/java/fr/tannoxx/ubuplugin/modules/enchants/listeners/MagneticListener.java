 ...
 * Items récupérés directement dans l'inventaire
 * Toggle avec Shift + Clic Droit
 */
public class MagneticListener implements Listener {

    private final EnchantsModule module;
    private final Map<UUID, Long> recentBreakers = new HashMap<>();
    ...
    /**
     * Toggle Magnetic avec Shift + Clic Droit
     */
    ...
    /**
     * Récupération automatique des items de blocs cassés
     */
    ...
    public void onBlockDropItem(@NotNull BlockDropItemEvent event) {
        ...
        // Vérifier si Magnetic est désactivé
        UUID uuid = player.getUniqueId();
        ...
        // Enregistrer le joueur comme casseur récent
            ...
            if (!leftover.isEmpty()) {
                for (ItemStack leftoverItem : leftover.values()) {
                    player.getWorld().dropItemNaturally(
                            event.getBlockState().getLocation().add(0.5, 0.5, 0.5),
                            leftoverItem
                    );
                }
            }
    ...
    /**
     * Récupération items des mobs tués
     */
    @EventHandler(priority = EventPriority.HIGHEST, ignoreCancelled = true)
    public void onEntityDeath(@NotNull EntityDeathEvent event) {
        ...
        for (ItemStack drop : drops) {
            HashMap<Integer, ItemStack> leftover = killer.getInventory().addItem(drop);

            if (!leftover.isEmpty()) {
                for (ItemStack leftoverItem : leftover.values()) {
                    event.getDrops().add(leftoverItem);
                }
            }
        }
    }
    ...
    /**
     * Intercepte les items qui spawn (pour blocs en chaîne)
     */
    ...
    public void onItemSpawn(@NotNull ItemSpawnEvent event) {
        ...
        if (nearestPlayer != null) {
            HashMap<Integer, ItemStack> leftover = nearestPlayer.getInventory().addItem(stack);
    ...
    /**
     * Trouve le joueur le plus proche avec Magnetic actif
     */
    ...
    /**
     * Nettoie les anciens casseurs
     */
    public void cleanup() {
        long currentTime = System.currentTimeMillis();
        recentBreakers.entrySet().removeIf(entry ->
                (currentTime - entry.getValue()) > COLLECTION_WINDOW * 3
        );
    }
}