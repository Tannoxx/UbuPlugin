...
public class LobbyChatModule extends Module {
    ...
    private void loadChatReplacements() {
        ...
        if (getConfigManager().contains(path)) {
            for (String key : getConfigManager().getKeys(path)) {
                String replacement = getConfigManager().getString(path + "." + key, "");
        ...
        debug("Chargé {} remplacements de chat", chatReplacements.size());
    }
